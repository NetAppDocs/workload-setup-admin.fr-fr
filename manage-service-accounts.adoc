...
---
= Gérez les comptes de service dans NetApp Workload Factory
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Vous pouvez également obtenir un jeton d'accès pour l'API de gestion Auth0 en effectuant un échange d'identifiants client. Consultez link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console Documentation de l'API"^] pour obtenir des informations sur la génération d'un jeton d'actualisation.
+
L'exemple curl montre comment vous pouvez prendre l'identifiant client et le secret et utiliser une API pour générer le jeton d'accès, qui est limité dans le temps. Le jeton offre plusieurs heures d'accès aux API NetApp Workload Factory.
...
----
+
Envoyez le résultat à https://netapp-cloud-account.auth0.com/oauth/token pour obtenir le jeton d'accès.
