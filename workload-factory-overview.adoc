...
connectivité du lien Lambda:: Utilisez la connectivité Lambda link pour établir une relation de confiance et une connectivité entre un compte NetApp Workload Factory et un ou plusieurs systèmes de fichiers Amazon FSx for ONTAP. Les liens vous permettent de surveiller et de gérer certaines fonctionnalités du système de fichiers directement à partir des appels d’API REST ONTAP qui ne sont pas disponibles via l’API Amazon FSx for ONTAP.
...
opérations de stockage EDA:: Rationalisez vos opérations de stockage Electronic Design Automation (EDA) dans NetApp Workload Factory en optimisant les performances, en intégrant votre IaC pour automatiser les paramètres de stockage et en garantissant la santé opérationnelle.
...
NOTE: Les administrateurs de compte peuvent activer ou désactiver les fonctionnalités d'IA. Si un administrateur les désactive, l'assistant IA Ask Me ne sera pas disponible. link:manage-genai-features.html["Découvrez comment gérer les fonctionnalités d'IA"].
...
link:https://docs.netapp.com/us-en/workload-fsx-ontap/links-overview.html["En savoir plus sur les liens"^]
...
link:well-architected-workloads.html["Apprenez-en davantage sur les charges de travail bien architecturées."]
